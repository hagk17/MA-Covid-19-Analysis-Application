...
daily_data <- read.xlsx("clean_data.xlsx", sheetIndex = 4)
daily_data %<>% select(-NA., -TOWN_ID)

weekly_data <- read.xlsx("clean_data.xlsx", sheetIndex = 5)
weekly_data %<>% select(-NA.)

census_data <- read.xlsx("clean_data.xlsx", sheetIndex = 6)
...
full_server <- function(input, output){
  ...
  w_general_data <- reactive({
    ...
    if(input$w_choice == "w_conf"){
      good_data <- weekly_data %>% filter(Type == "Conf")}
    else{good_data <- weekly_data %>% filter(Type == "Rate")}
  ...
  w_left_hchart <- reactive({
    ...
    lw_hchart <- hchart(left_data, "line", hcaes(x = Time, y = Count, group = Town))%>%
      ...
      #hc_subtitle(text = "Covid Data Last Updated July 29, 2020", align = "left") %>%
      hc_legend(enabled = F)
